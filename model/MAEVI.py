...
class UNet_3D_3D(nn.Module):
    def __init__(self, n_inputs=4, joinType="concat", ks=5, dilation=1, voxelGridSize = 128, nf_out = 64):
        ...
        self.blur0 = T.GaussianBlur(kernel_size=13)
        self.blur1 = T.GaussianBlur(kernel_size=7)
        self.blur2 = T.GaussianBlur(kernel_size=3)
        ...
        self.parallel_ll = SynBlock(n_inputs, nf_out, ks=ks, dilation=dilation, norm_weight=False)
        self.parallel_l = SynBlock(n_inputs, nf_out, ks=ks, dilation=dilation, norm_weight=False)
        self.parallel = SynBlock(n_inputs, nf_out, ks=ks, dilation=dilation, norm_weight=False)

        self.predict_ll = SynBlock(n_inputs, nf_out, ks=ks, dilation=dilation, norm_weight=False)
    ...
    def forward(self, frames, voxel):
        ...
        abs_vox = torch.abs(voxel)
        sum_vox = torch.sum(abs_vox,1)
        normed = torch.nn.functional.normalize(sum_vox)

        
        blurred = self.blur0(normed)
        blurred = self.blur1(blurred)
        blurred = self.blur2(blurred)
        
        region = list(torch.mul(torch.unsqueeze(blurred,-1) , torch.stack(frames).permute(1,0,3,4,2)).permute(1,0,4,2,3))
        #! region = [torch.unsqueeze(a,0) for a in region]
        ...
        out_ll_0 = self.predict_ll(fea3, frames, fea2.size()[-2:]) 

        out_l_0 = self.predict_l(fea2, frames, fea1.size()[-2:])
        out_l_0 = F.interpolate(out_ll_0, size=out_l_0.size()[-2:], mode='bilinear') + out_l_0

        out_0 = self.predict(fea1, frames, (H,W))
        out_0 = F.interpolate(out_l_0, size=out_0.size()[-2:], mode='bilinear') + out_0

        out_ll_1 = self.parallel_ll(fea3, region, fea2.size()[-2:]) 

        out_l_1 = self.parallel_l(fea2, region, fea1.size()[-2:])
        out_l_1 = F.interpolate(out_ll_1, size=out_l_1.size()[-2:], mode='bilinear') + out_l_1

        out_1 = self.parallel(fea1, region, (H,W))
        out_1 = F.interpolate(out_l_1, size=out_1.size()[-2:], mode='bilinear') + out_1

        out_ll = out_ll_0 + out_ll_1
        out_l = out_l_0 + out_l_1
        out = out_0 + out_1

        if self.training:
            return region,out_ll, out_l, out
        else:
            return out
