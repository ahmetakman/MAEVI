...
save_loc = os.path.join(args.checkpoint_dir, "checkpoints",local_branch,str(marker.day)+"_"+str(marker.month))
...
#     train_set = EventVoxel(args.data_root, args.sparse_data_root, is_hsergb=False, is_training=True, is_validation=False, number_of_time_bins= args.voxel_grid_size)
#     validation_set = EventVoxel(args.data_root, args.sparse_data_root, is_hsergb=False, is_training=False, is_validation=True, number_of_time_bins= args.voxel_grid_size)
#     train_loader = DataLoader(train_set, args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
#     validation_loader = DataLoader(validation_set, args.test_batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
# else:
...
def train(args, epoch):
    ...
    for i, (images, voxel, gt_image, path) in enumerate(tqdm(train_loader)):
        ...
        # Forward
        optimizer.zero_grad()
        region,out_ll, out_l, out = model(images, voxel)

        gt = gt_image.to(device)

        gt_r = (region[1]+region[2])*gt
        out_r = (region[1]+region[2])*out
        loss_r,_ = criterion(out_r,gt_r) 
        loss, _ = criterion(out, gt)
        overall_loss = loss*0.4 + loss_r*0.6

        losses['total'].update(loss.item()*0.4+loss_r.item()*0.6)
...
lr_schular = [16e-4, 8e-4, 4e-4, 2e-4, 1e-4, 1e-4, 1e-4, 5e-5,2.5e-5]
# training_schedule = [2, 4, 6, 8, 10, 12]
training_schedule = [10, 16, 24, 30, 34, 40, 50, 56,62]
